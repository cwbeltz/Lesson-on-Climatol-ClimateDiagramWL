...
Reading data directly into R (only for pre-calculated data):

*Note: For the Kingfisher data, I did all the data manipulation as we did in class on September 13, 2017. Running the code chunk below will read the data, in the correct format, directly into R from my Dropbox.*
...
```{r, eval=F}
#open PDF
pdf("Beltz_2017_FES634_Walter.Lieth_Kingfisher.OK_DIAGRAM.pdf", width=8.0, height=6.0)

diagwl(Kingfisher.climdat, est="Kingfisher, OK", alt=320, per="1981-2010", mlab="en")
...
Remove all files from Example 1:

```{r}
rm(Kingfisher.climdat)
```
