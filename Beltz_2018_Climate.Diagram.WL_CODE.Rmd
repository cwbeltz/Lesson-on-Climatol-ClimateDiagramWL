...
*Note: R has added an X in front of the numbers that reflect the month. This is not a problem, at least for this use case, and we can ignore it. This is a common occurence when using `read.csv` to read in data that has columns names that begin with or are a number.*
